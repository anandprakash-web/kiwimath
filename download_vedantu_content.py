#!/usr/bin/env python3
"""Download the Vedantu olympiad ASSIGNMENT PDFs (L5/L6/L7, 140 files,
your academic team's own content) into ~/Downloads/Vedantu_Content, organised by
level/pillar/topic. Resumable: skips anything already downloaded.

  1)  pip install gdown
  2)  python3 download_vedantu_content.py

If some files are permission-restricted (not "anyone with link"), they'll be listed
at the end with their URLs — open those once in your browser (you're signed in), or
use rclone. Reference TEXTBOOKS are intentionally NOT included (3rd-party copyright).
"""
import os, sys, subprocess

def _ensure_gdown():
    try:
        import gdown  # already installed
        return
    except ImportError:
        pass
    print("Installing gdown (one-time, ~10s)...")
    attempts = (
        ["-m", "pip", "install", "--user", "gdown"],
        ["-m", "pip", "install", "--user", "--break-system-packages", "gdown"],
    )
    for a in attempts:
        try:
            subprocess.check_call([sys.executable] + a); break
        except Exception:
            continue
    else:
        try:
            ...
            sys.exit("Could not auto-install gdown.\nRun this once:  python3 -m pip install --user gdown\nthen re-run this script.")
    # restart so the freshly installed gdown is importable
...
def _fetch(fid, out, tries=4):
    import time
    for i in range(tries):
        try:
            r = gdown.download(id=fid, output=out, quiet=True, fuzzy=True)
        ...
        time.sleep(4 * (i + 1))   # back off when Google throttles anonymous access
    return False
...
def main():
    ...
    for n, (lvl, pillar, topic, fid) in enumerate(ASSIGNMENTS, 1):
        ...
        if _fetch(fid, out):
            ...
            failed.append((lvl, pillar, topic, fid)); print("   ! throttled/failed (will retry on next run)")
        time.sleep(1.0)   # be gentle so Google doesn't throttle
    print(f"\n=== {ok} downloaded, {skip} already present, {len(failed)} still missing ===")
    print("Folder: " + BASE)
    if failed:
        print("\n%d not yet downloaded. RE-RUN this script in a minute (it resumes/skips done)." % len(failed))
        print("Anything still stuck after 2-3 runs is genuinely restricted - open in browser, or ask Claude for the rclone version:")
