...
# ── L3 additions: angles, ratios, coordinates ───────────
def angle(deg, label=None, col=INK):
    cx, cy, r = 56, 150, 132
    rad = math.radians(deg)
    x2, y2 = cx + r*math.cos(rad), cy - r*math.sin(rad)
    ar = 40
    ax2, ay2 = cx + ar*math.cos(rad), cy - ar*math.sin(rad)
    s = [f'<line x1="{cx}" y1="{cy}" x2="{cx+r}" y2="{cy}" stroke="{col}" stroke-width="2.6"/>',
         f'<line x1="{cx}" y1="{cy}" x2="{x2:.0f}" y2="{y2:.0f}" stroke="{col}" stroke-width="2.6"/>',
         f'<path d="M{cx+ar},{cy} A{ar},{ar} 0 0 0 {ax2:.0f},{ay2:.0f}" fill="none" stroke="{ORANGE}" stroke-width="2.2"/>',
         f'<circle cx="{cx}" cy="{cy}" r="3.5" fill="{col}"/>',
         _t(cx+ar+20, cy-22, label or f"{deg}&#176;", 16, ORANGE, "start", 800)]
    return _svg("".join(s), 210, 172)


def ratio_bar(parts, labels=None, w=380):
    tot = sum(parts) or 1; x = 10; s = []; cols = [SKY, BERRY, GRASS, ORANGE, PURPLE]
    for i, p in enumerate(parts):
        seg = (w-20)*p/tot
        s.append(f'<rect x="{x:.1f}" y="16" width="{seg:.1f}" height="42" fill="{cols[i%5]}" fill-opacity="0.62" stroke="{cols[i%5]}" stroke-width="1.8"/>')
        s.append(_t(x+seg/2, 43, str(labels[i]) if labels else str(p), 16, "#fff", w=800))
        x += seg
    return _svg("".join(s), w, 74)


def coord_grid(points, n=6):
    c, o = 28, 26
    s = [f'<rect x="{o}" y="6" width="{n*c}" height="{n*c}" fill="none" stroke="{INK}" stroke-width="1.6"/>']
    for k in range(n+1):
        s.append(f'<line x1="{o+k*c}" y1="6" x2="{o+k*c}" y2="{6+n*c}" stroke="{INK}" stroke-width=".6" opacity=".4"/>')
        s.append(f'<line x1="{o}" y1="{6+k*c}" x2="{o+n*c}" y2="{6+k*c}" stroke="{INK}" stroke-width=".6" opacity=".4"/>')
        s.append(_t(o+k*c, 6+n*c+15, str(k), 11))
        if k < n: s.append(_t(o-8, 6+(n-k)*c+4, str(k), 11, INK, "end"))
    for (px, py, lab) in points:
        cx, cy = o+px*c, 6+(n-py)*c
        s.append(f'<circle cx="{cx}" cy="{cy}" r="6" fill="{ORANGE}"/>')
        s.append(_t(cx+9, cy-7, str(lab), 12, ORANGE, "start", 800))
    return _svg("".join(s), o+n*c+18, 6+n*c+24)
