...
# olympiad
r=c.get("/v3/olympiad/levels").json(); P(len(r["levels"])==8 and r["levels"][0]["available"] and not r["levels"][3]["available"], "levels: 8, L1 avail, L4 not")
...
el=c.get("/v3/olympiad/levels/L4/topics").json(); P(len(el["topics"])>0 and all(not t["available"] for t in el["topics"]), "empty level L4 -> all topics coming-soon")
...
st=c.get("/v3/stats").json(); P(st["olympiad_total"]==18099 and st["curriculum_total"]==10340, "stats match the bank")
print("\nDONE")
